...
func DefaultPath() string {
	if p := os.Getenv("WEORC"); p != "" {
		return p
	}
	if runtime.GOOS == "windows" {
		return filepath.Join(Dir(), "weorc")
	}
	return filepath.Join(HomeDir(), ".weorc")
}
...
func (c *Config) Add(s *Cluster, force bool) error {
	...
	for i, existing := range c.Clusters {
		...
		case existing.Name == s.Name:
			m = fmt.Sprintf("Cluster %q already exists in ~/.weorc", s.Name)
		case existing.GitURL != "" && existing.GitURL == s.GitURL:
			m = fmt.Sprintf("A cluster with the URL %q already exists in ~/.weorc", s.GitURL)
		case existing.ControllerURL == s.ControllerURL:
			m = fmt.Sprintf("A cluster with the URL %q already exists in ~/.weorc", s.ControllerURL)
		case existing.DockerPushURL != "" && existing.DockerPushURL == s.DockerPushURL:
			m = fmt.Sprintf("A cluster with the URL %q already exists in ~/.weorc", s.DockerPushURL)
		}
		if m != "" {
